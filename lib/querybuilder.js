...
QueryBuilder.prototype.order = function (field, asc) {
  if (this.queryType !== 3) return this;

  asc = asc === false || (typeof(asc) === 'string' && asc.toLowerCase() === 'desc') ? 'DESC' : 'ASC';
...
QueryBuilder.prototype.getQuery = function () {
  ...
  if (_this.queryType === 1) {
    ...
    if (_this.queryType === 3) {
      if (_this._order) query += ' ORDER BY ' + _this._order;
